...
# Discriminator: D(vi_scaled, v_r_scaled, class) -> prob
...
# Build synthetic ballistic dataset (use uniform sampling of vi)
def make_ballistic_dataset(classes_vbl, samples_per_class=200, vi_max=1200.0, noise_std=6.0):
    X = []
    Y = []
    for class_idx, v_bl in enumerate(classes_vbl):
        vi = np.random.uniform(0.0, vi_max, size=samples_per_class)
        vi.sort()
        vr = lambert_jonas(vi, v_bl=v_bl)
        vr = vr + np.random.normal(0.0, noise_std, size=vr.shape)
        vr = np.clip(vr, 0.0, None)
        for a, b in zip(vi, vr):
            X.append([a, b])
            Y.append(class_idx)
    X = np.array(X, dtype=np.float32)
    Y = np.array(Y, dtype=np.int32)
    return X, Y, classes_vbl

# Training + generation + Lambert fitting pipeline
def train_and_generate_with_fit(classes_vbl=None,samples_per_class=800,vi_max=1200.0,latent_dim=32,batch_size=64,epochs=3000,seed=42,class_to_generate=0,save_excel=False,extrapolation_factor=1.5):
    if classes_vbl is None:
        classes_vbl = [200, 500, 600, 700, 800]
    ...
    # Extract ORIGINAL training data for selected class
    ...
    def generate_for_class(class_id, n_points=3000, extend_factor=1.0):
        vi_max_extended = vi_max * extend_factor
        vi_sweep = np.linspace(0.0, vi_max_extended, n_points).astype(np.float32).reshape(-1,1)
    ...
    print(f"Extrapolating to {extrapolation_factor}x training range (up to {vi_max * extrapolation_factor:.0f} m/s)")
    vi_gen, vr_gen = generate_for_class(class_to_generate, n_points=4000, extend_factor=extrapolation_factor)
    # Fit Lambert–Jonas to generated samples
    mask = vr_gen > 0.0
    vi_fit = vi_gen[mask]
    vr_fit = vr_gen[mask]
    # initial guess: a=1.0, p=3.0, v_bl ~ class center
    p0 = [1.0, 3.0, float(classes_vbl[class_to_generate])]
    lb = [1e-4, 1.0, 0.0]
    ub = [20.0, 10.0, vi_max * extrapolation_factor * 1.05]
    ...
    step = max(1, len(vi_train)//300)
    for i in range(0, len(vi_train),step):
      print(f"{vi_train[i]:10.2f} {vr_train[i]:18.2f} {vr_gen[i]:18.2f}")

    plt.figure(figsize=(11, 7))
    # GAN predictions at same v_i
    plt.scatter(vi_train, vr_gan_on_train_vi, s=28, color="red", alpha=0.55, label="GAN prediction (same $v_i$)")
    # GAN predictions in extrapolation region
    extrapolation_mask = vi_gen > vi_max
    if np.any(extrapolation_mask):
        plt.scatter(vi_gen[extrapolation_mask], vr_gen[extrapolation_mask],s=20, color="orange", alpha=0.4,label="GAN extrapolation (beyond training)")
    print("  v_i (predicted)", "   v_r (predicted)")
    for i in range(0, len(vr_gen[extrapolation_mask]),25):
      print(f"     {vi_gen[extrapolation_mask][i]:1.2f}             {vr_gen[extrapolation_mask][i]:1.2f}")

    # True Lambert–Jonas curve (extended range)
    ...
    plt.axvline(vi_max,color="purple", linestyle=":", linewidth=2.0, alpha=0.7,label=f"Training range limit ({vi_max:.0f} m/s)")
    ...
    plt.title(f"GAN → Lambert–Jonas Fit with Extrapolation | Class {class_to_generate}\n" f"True V_bl={vbl_true:.0f} m/s, Predicted V_bl={vbl_pred:.1f} m/s | " f"Extrapolated to {vi_max * extrapolation_factor:.0f} m/s", fontsize=13)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    results = train_and_generate_with_fit(classes_vbl=[300, 500, 600, 700, 800],samples_per_class=1500,vi_max=500.0,latent_dim=32,batch_size=64,epochs=5000,seed=2025,extrapolation_factor=1.5,class_to_generate=0)
